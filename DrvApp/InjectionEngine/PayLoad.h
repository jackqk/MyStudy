...
namespace ddk::injection {
	extern const unsigned char mem_load32_shellcode[0xD20];
	static const int THREAD_CREATE_FLAGS_CREATE_SUSPENDED = 0x00000001;
	static const int THREAD_CREATE_FLAGS_SKIP_THREAD_ATTACH = 0x00000002;
	static const int THREAD_CREATE_FLAGS_HIDE_FROM_DEBUGGER = 0x00000004;
	class PayLoad
	{
	public://构造
		PayLoad() {
	...
	public://操作
		static PVOID AllocateInjectMemory(IN HANDLE ProcessHandle, IN PVOID DesiredAddress, IN SIZE_T DesiredSize)
		{
			...
			while (1)
			{
				if (!NT_SUCCESS(FGNtQueryVirtualMemory(ProcessHandle, DesiredAddress, MemoryBasicInformation, &mbi, sizeof(mbi), NULL)))
				{
		...
		static BOOL  MakeShellcodeLoadDll(HANDLE ProcessId, PVOID *out_mem, std::wstring dll_path)
		{
...
#pragma warning( pop )
			...
			ns = FGNtWriteVirtualMemory(ProcessHandle, pBuffer, &Buffer, sizeof(Buffer), NULL);
			if (!NT_SUCCESS(ns))
			{
				return false;
			}
		...
		static BOOL  MakeShellcodeMemLoad32(HANDLE ProcessId, PVOID *out_mem, PVOID file_data, DWORD file_size)
		{
			PEPROCESS Process = nullptr;
			auto ns = PsLookupProcessByProcessId(ProcessId, &Process);
			if (!NT_SUCCESS(ns))
			...
			DWORD ShellCodeSize = sizeof(mem_load32_shellcode);
			...
			auto mem_shellcode = (PBYTE)malloc(sizeof(mem_load32_shellcode));
			if (!mem_shellcode)
			{
				return false;
			}
			...
			RtlCopyMemory(mem_shellcode, mem_load32_shellcode, sizeof(mem_load32_shellcode));

			int flag = 0xAFAFAFAF;

			for (UINT i = 0; i < sizeof(mem_load32_shellcode); i++)
			{
...
#define FIND_FLAG 0xAFBFAFBF
			...
			ns = FGNtWriteVirtualMemory(ProcessHandle, shellcode, p_mem, TotolSize, &ReturnLength);
			if (!NT_SUCCESS(ns))
			{
				return false;
			}
		...
		static BOOL  ExecuteThread(HANDLE ProcessId, PVOID ThreadRoutine, PVOID param)
		{
			PEPROCESS Process = nullptr;
			auto ns = PsLookupProcessByProcessId(ProcessId, &Process);
			if (!NT_SUCCESS(ns))
			...
			auto status = FGNtCreateThreadEx(
				&hThread, THREAD_ALL_ACCESS, NULL,
				ProcessHandle, ThreadRoutine, param, FALSE,
	...
	public://PE相关操作
		...
		static PVOID BBGetModuleExport(IN PVOID pBase, IN PCHAR name_ord)
		{
