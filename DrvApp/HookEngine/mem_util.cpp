...
namespace Detours::X64::mem_util
{
#ifdef _AMD64_
...
#pragma warning(pop)
	...
	void init_mem_util()
	{
		...
		const auto p_MmGetVirtualForPhysical = ddk::util::SyscallUtil::getIntanceRef().getSysCall("MmGetVirtualForPhysical");
	...
	bool UtilIsNonPageableAddress(void *address)
	{
		...
		bool is_v6_kernel = !ddk::util::OsVerUtil::IsWin10();
	...
	bool alloc_mem(PVOID wanna_address, size_t alloc_size, PMM_SYSTEM_ALLOC _allocRet)
	{
		...
		{
			alloc_address = PVOID((PUCHAR)imageBase + ddk::util::PEUtil::LdrGetImageSize(imageBase) + PAGE_SIZE);
		}
