...
#define ALIGN_32(x)			Internal::AlignAddress((uint64_t)(x), 4);
	...
	{
		// Decode the data
		_DecodedInst decodedInstructions[30];
		uint32_t decodedCount = 0;

		_DecodeResult res = distorm_decode((_OffsetType)Target, (const unsigned char *)Target, 32, Decode32Bits, decodedInstructions, 30, &decodedCount);
		...
		header->Random = (uint32_t)PsGetCurrentProcessId() + (uint32_t)PsGetCurrentThreadId() + totalInstrSize;
