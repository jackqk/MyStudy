...
namespace Detours::Internal
{
	...
	bool AtomicCopy4X8(uint8_t *Target, uint8_t *Memory, sizeptr_t Length)
	{
		...
		if (!VirtualProtect(Target, Length, PAGE_EXECUTE_READWRITE, &dwOld))
			...
			//MDL
		ddk::util::MM_SYSTEM_MAP _map = {};
		if (!ddk::util::MemUtil::MmMapSystemMemoryWritable(Target, 8, &_map))
		{
			return false;
		}
		auto exit_map = std::experimental::make_scope_exit([&]() { ddk::util::MemUtil::MmFreeMap(&_map); });

		auto _NewTarget = _map.pMappedAddress;
		//CPULOCK
		//DPC
		KIRQL oldirql;
		ddk::lock::CCpuLock _cpu;
		_cpu.lock();
	...
	bool WriteMemory(uint8_t *Target, uint8_t *Memory, sizeptr_t Length)
	{
		DWORD dwOld = 0;
		ddk::util::MM_SYSTEM_MAP _map = {};
		if (!ddk::util::MemUtil::MmMapSystemMemoryWritable(Target, Length, &_map))
		{
			return false;
		}
		auto exit_map = std::experimental::make_scope_exit([&]() { ddk::util::MemUtil::MmFreeMap(&_map); });
