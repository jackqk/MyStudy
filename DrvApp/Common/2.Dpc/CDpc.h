...
namespace ddk
{
	...
	public:
		...
		void _Launch(CDpc *work)
		{
			_dpc = work;
			//
			auto context = work->dpc_ctx;
			auto i = work->g_run_cpu;
			KeInitializeDpc(&context->Dpcs[i], _launch_callback_dpc, this);
	...
	template<class _Target> inline
		void _Launch_dpc(CDpc *_Thr, _Target&& _Tg)
	{	// launch a new thread
	...
	public:
		...
		template<class _Fn,
			...
			!std::is_same<typename std::decay<_Fn>::type, CDpc>::value>::type>
			explicit CDpc(_Fn&& _Fx, _Args&&... _Ax)
