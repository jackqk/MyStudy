...
namespace ddk::net::wsk
{
		...
		PWSK_SOCKET	NTAPI SocketConnect(__in USHORT SocketType,	__in ULONG Protocol,
		...
		LONG NTAPI Receive(__in  PWSK_SOCKET WskSocket,	__out PVOID Buffer,
				           __in  ULONG BufferSize,	__in  ULONG Flags);
		LONG NTAPI ReceiveFrom(__in  PWSK_SOCKET WskSocket,	__out PVOID Buffer, __in  ULONG BufferSize,
					           __out_opt PSOCKADDR RemoteAddress, __out_opt PULONG ControlFlags);
		...
		NTSTATUS NTAPI ResolveName(_In_ PUNICODE_STRING NodeName, _In_ PUNICODE_STRING ServiceName,
			                      _In_opt_ PADDRINFOEXW Hints, __out PSOCKADDR_IN ResolvedAddress	);
		NTSTATUS NTAPI DisConnect(_In_ PWSK_SOCKET	WskSocket);
};