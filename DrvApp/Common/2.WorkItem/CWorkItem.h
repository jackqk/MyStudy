...
namespace ddk
{
	...
	public:
		...
		void _Launch(CWorkItem *work)
		{
			wk = work->get_work_item();
			ExInitializeWorkItem(wk, _launch_callback_WI, this);
	...
	template<class _Target> inline
		void _Launch_work_item(CWorkItem *_Thr, _Target&& _Tg)
	{	// launch a new thread
	...
	public:
		...
		template<class _Fn,
			...
			!std::is_same<typename std::decay<_Fn>::type, CWorkItem>::value>::type>
			explicit CWorkItem(WORK_QUEUE_TYPE w_type, _Fn&& _Fx, _Args&&... _Ax)
