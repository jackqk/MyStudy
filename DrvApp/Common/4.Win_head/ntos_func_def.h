...
#ifndef SYSTEM_HANDLE_TABLE_ENTRY_INFO//句柄表
...
#ifndef SYSTEM_HANDLE_TABLE_ENTRY_INFO_EX//句柄表
...
#pragma pack()
	...
	NTSYSAPI VOID NTAPI KeGenericCallDpc(
			_In_ PKDEFERRED_ROUTINE Routine,
			_In_opt_ PVOID Context);

	NTSYSAPI VOID NTAPI	KeSignalCallDpcDone(_In_ PVOID SystemArgument1);
	NTSYSAPI LOGICAL NTAPI KeSignalCallDpcSynchronize(_In_ PVOID SystemArgument2);
	NTSYSAPI PVOID NTAPI KeQueryPrcbAddress(__in ULONG Number);
	...
	NTSYSAPI PCCHAR   NTAPI PsGetProcessImageFileName(IN PEPROCESS Process);
	...
	NTSYSAPI PVOID    NTAPI PsGetThreadTeb(IN PETHREAD Thread);
	NTSYSAPI BOOLEAN  NTAPI PsIsThreadTerminating(__in PETHREAD Thread);
	...
	NTSYSAPI BOOLEAN NTAPI KeInsertQueueApc(
			PKAPC Apc,
			PVOID SystemArgument1,
			PVOID SystemArgument2,
			KPRIORITY Increment);

	NTSYSAPI BOOLEAN NTAPI KeTestAlertThread(IN KPROCESSOR_MODE AlertMode);
	NTSYSAPI PVOID NTAPI PsGetProcessWin32Process(PEPROCESS Process);
