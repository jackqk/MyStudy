...
extern "C" int __cdecl atexit(__in void(__cdecl *destructor)(void)
)
{
	if (!destructor)
		return 0;
...
extern "C" void __cdecl cc_doexit(__in int, __in int,__in int)
{
...
extern "C" int __cdecl cc_init(__in int)
{
	//STL BOOST！
...
__drv_maxIRQL(DISPATCH_LEVEL)
void __cdecl free(__inout void* ptr)
{
	if (ptr)
...
extern "C" __drv_maxIRQL(DISPATCH_LEVEL) void* __cdecl calloc(__in size_t n,__in size_t size)
{
	if (!size)
		return nullptr;
	if ((size_t)(~0) / n < size)
