#pragma once
//#define LOGSTATUS_BREAK(status) ddk::log::StatusInfo::Print(status);
...
namespace ddk {
	namespace log {
...
#endif
		public:
			static void Print(NTSTATUS code) {
#if defined(DBG)
				for (int i = 0; i < m_statusTableLength; i++)
				{
					if (m_statusTable[i].m_code == (ULONG)code) {
						LOG_DEBUG("%x：%s（%s）\r\n", m_statusTable[i].m_code,
							m_statusTable[i].m_name,
							m_statusTable[i].m_description);
						break;
					}
				}
#endif // DBG	
		...
		private:
#if defined(DBG)
			static const ULONG m_statusTableLength = 2000;	//这个随便先指定，以后增加还的加
			static const inline Info m_statusTable[m_statusTableLength] = {
			...
			{ 0x4000000E,     "STATUS_IMAGE_MACHINE_TYPE_MISMATCH", "The image file %hs is valid, but is for a machine type other than the current machine. Select OK to continue, or CANCEL to fail the DLL load."},
