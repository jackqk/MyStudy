...
namespace Detours
{
	namespace X64
	{
		...
		uint8_t *DetourFunction(uint8_t *Target, uint8_t *Detour, X64Option Options)
		{
			...
			uint8_t *jumpTrampolinePtr = (uint8_t *)alloc_mem((PVOID)Target, allocSize);
			...
			header->InstructionOffset = jumpTrampolinePtr + sizeof(JumpTrampolineHeader);//ALIGN_64(jumpTrampolinePtr + sizeof(JumpTrampolineHeader));
		...
		bool DetourRemove(uint8_t *Trampoline)
		{
			...
			DWORD dwOld = 0;
			if (!VirtualProtect(header->CodeOffset, header->InstructionLength, PAGE_EXECUTE_READWRITE, &dwOld))
				return false;
			memcpy(header->CodeOffset, header->InstructionOffset, header->InstructionLength);
			VirtualProtect(header->CodeOffset, header->InstructionLength, dwOld, &dwOld);
